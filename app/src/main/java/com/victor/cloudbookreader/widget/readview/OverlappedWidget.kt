...
class OverlappedWidget(context: Context, bookId: String,
    ...
    init {
        ...
        mBackShadowDrawableRL = GradientDrawable(GradientDrawable.Orientation.RIGHT_LEFT,
                mBackShadowColors)
        mBackShadowDrawableRL.gradientType = GradientDrawable.LINEAR_GRADIENT

        mBackShadowDrawableLR = GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT,
                mBackShadowColors)
    ...
    override fun drawCurrentPageShadow(canvas: Canvas) {
        ...
        if (actiondownX > mScreenWidth shr 1) {
            ...
            shadow.setBounds((mScreenWidth + touch_down - 5).toInt(), 0, (mScreenWidth.toFloat() +

                    touch_down + 5f).toInt(), mScreenHeight)
    ...
    override fun computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (actiondownX > mScreenWidth shr 1) {
                touch_down = (-(mScreenWidth - x)).toFloat()
            } else {
