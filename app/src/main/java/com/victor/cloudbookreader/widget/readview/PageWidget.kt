...
class PageWidget(context: Context, bookId: String,
    ...
    internal var mBackShadowColors: IntArray? = null// 背面颜色组
    internal var mFrontShadowColors: IntArray? = null// 前面颜色组

    internal var mBackShadowDrawableLR: GradientDrawable? = null // 有阴影的GradientDrawable
    internal var mBackShadowDrawableRL: GradientDrawable? = null
    internal var mFolderShadowDrawableLR: GradientDrawable? = null
    internal var mFolderShadowDrawableRL: GradientDrawable? = null
    internal var mFrontShadowDrawableHBT: GradientDrawable? = null
    internal var mFrontShadowDrawableHTB: GradientDrawable? = null
    internal var mFrontShadowDrawableVLR: GradientDrawable? = null
    internal var mFrontShadowDrawableVRL: GradientDrawable? = null
     ...
     * @param canvas
     */
    public override fun drawCurrentPageShadow(canvas: Canvas) {
        val degree: Double
        if (mIsRTandLB) {
    ...
    override fun computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            val x = mScroller.getCurrX()
            val y = mScroller.getCurrY()
            mTouch.x = x.toFloat()
            mTouch.y = y.toFloat()
            postInvalidate()
        }
    ...
    public override fun abortAnimation() {
        if (!mScroller.isFinished()) {
            mScroller.abortAnimation()
        }
    }

    public override fun restoreAnimation() {
        val dx: Int
        val dy: Int
        if (mCornerX > 0) {
            dx = (mScreenWidth - mTouch.x).toInt()
    ...
    @Synchronized override fun setTheme(theme: Int) {
        //        resetTouchPoint();
