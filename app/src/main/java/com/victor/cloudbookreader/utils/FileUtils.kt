...
class FileUtils {
     ...
     * @param ext  指定扩展名
     */
    @Synchronized private fun getAllFiles(root: File, ext: String) {
    ...
    companion object {
        ...
        fun getBookDir(bookId: String): File {
            return File(Constants.PATH_TXT + bookId)
        }

        fun createWifiTempFile(): File {
            val src = Constants.PATH_DATA + "/" + System.currentTimeMillis()
            val file = File(src)
            if (!file.exists())
                createFile(file)
            return file
        ...
        fun createWifiTranfesFile(fileName: String): File {
            // 取文件名作为文件夹（bookid）
            val absPath = Constants.PATH_TXT + "/" + fileName + "/1.txt"
...
//        }
        ...
        fun readAssets(fileName: String?): ByteArray? {
            ...
            try {
                val fin = ReaderApplication.readerApplication.assets.open("uploader" + fileName)
